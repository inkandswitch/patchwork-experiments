...
export function EmojiPicker(props: {
	ctx: SlotContextValue
	targetIdx: number | null
	anchorEl: HTMLElement | null
	onClose: () => void
}) {
	// Context comes in explicitly (via SlotContext) so this component holds no
	// useContext dependency and can live in another bundle.
	const {handle, doc, selector} = props.ctx.chat
	const {myName, myEmoticons, setMyEmoticons, chatProfileHandle} = props.ctx.identity
	const {peerEmoticons} = props.ctx.presence
	...
	async function selectEmoji(emoji: string) {
		...
		if (entry.ref && entry.url) {
			try {
				const repo = props.ctx.chat.repo
				if (!repo) return
