...
export function EmoticonAddDialog(props: {
	...
	// Seed this bundle's repo singleton (file-helpers/blob-cache use it).
	setRepo(props.ctx.chat.repo)
	const {handle} = props.ctx.chat
	const {myEmoticons, setMyEmoticons, chatProfileHandle, myName} = props.ctx.identity
	const {broadcastPresence} = props.ctx.presence
