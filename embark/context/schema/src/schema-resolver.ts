...
import { extractDocLinks } from "./doc-links";
...
// invisible — a container keeps helper documents (a spec, a folder of files)
// only to implement itself, and those must never surface as canvas matches. For
// these types we don't follow the container's links into its internals, AND we
// exclude those internal docs from matching even when mounted directly. The
// container itself stays matchable; only its contents are ignored. (Currently
// empty; kept so a future opaque container type can opt in.)
const OPAQUE_CONTAINER_TYPES = new Set<string>([]);
