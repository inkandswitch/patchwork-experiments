...
// dynamically-loaded card code) share one context with the Cards sidebar and
// every other canvas. Discovery from an embed finds no enclosing host and
// falls back to the body store (see findContextStore).
export const EmbarkCanvasTool: ToolRender = (handle, element) =>
  mountCanvas(handle as DocHandle<EmbarkCanvasDoc>, element);

// Promote the host so absolutely-positioned embeds have a positioned ancestor
// and render the canvas into `element`. Returns a disposer. The canvas takes
// no part in schema matching — document discovery and matching live in the
// Open Documents and Schema Matcher cards (see @embark/schema), wired through
// the `OpenDocuments` channel.
function mountCanvas(
  handle: DocHandle<EmbarkCanvasDoc>,
  element: ToolElement,
): () => void {
  logDnd("mountCanvas: dnd diagnostics active", { docUrl: handle.url });
...
function EmbarkCanvas(props: { handle: DocHandle<EmbarkCanvasDoc> }) {
  ...
  const dropDocuments = (event: DragEvent) => {
    logDnd("canvas drop: reached canvas root handler", {
    ...
    if (!payload) {
      logDnd("canvas drop: no payload extracted — drop ignored");
      return;
    }
    ...
    logDnd("canvas drop: inserting", { payload, dropX, dropY });
    ...
    <div
      ...
      on:dragover={(event) => {
        if (!hasDocumentDrag(event.dataTransfer)) {
          logDndOnce("canvas dragover: rejected (no known drag type)", {
        ...
        logDndOnce("canvas dragover: accepted", {
          types: event.dataTransfer
            ? Array.from(event.dataTransfer.types)
            : null,
      ...
      oncapture:drop={(event) => {
        // Capture phase sees every drop inside the canvas subtree, even ones an
        // embed's bubble handler will stop — logs whether the drop arrived at
        // all vs. got swallowed before the root's bubble handler.
        logDnd("canvas drop (capture phase): drop landed inside canvas", {
          target: describeElement(event.target),
          insideEmbed: Boolean(
            (event.target as Element | null)?.closest?.(".embark-embed"),
          ),
          types: event.dataTransfer
            ? Array.from(event.dataTransfer.types)
            : null,
...
function EmbedView(props: {
  ...
  return (
    <div
      ...
      on:dragover={(event) => {
        logDndOnce("embed dragover: stopping propagation (canvas won't see it)", {
          embedId: props.embed.id,
          toolId: toolId(),
        });
        event.stopPropagation();
      }}
      ...
      on:drop={(event) => {
        logDnd("embed drop: swallowed by embed (stopPropagation)", {
          embedId: props.embed.id,
          toolId: toolId(),
...
// Compact one-line description of an event target for the diagnostic dnd logs
// (tag + classes + closest embed id), so log entries stay readable.
