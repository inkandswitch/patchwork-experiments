...
export type Channel<T extends Record<string, unknown>> = {
  ...
  key?: string;
  value?: string;
  // Attribution: the module URL of the channel's canonical definition
  // (`import.meta.url` in the owning package's channels.js) and the URL of the
  // spec that documents its contract. Purely informational — identity stays
  // structural (by `name`) — but inspectors and the card-regeneration loop use
  // these to answer "who defined this channel and where is it documented".
  definedBy?: string;
  spec?: string;
};
...
export function createContextStore(): ContextStore {
  ...
  const stateFor = (channel: Channel<AnyRecord>): ChannelState => {
    ...
    if (!state) {
      ...
      notifyChannels();
    } else if (channel.definedBy && !state.channel.definedBy) {
      // Upgrade to the attributed definition: whichever caller passes the
      // canonical def (carrying `definedBy`) wins over an unattributed copy
      // seen earlier, so `channels()` reports provenance regardless of the
      // order writers and readers arrive in.
...
export function findContextStore(node: Node): ContextStore {
  if (!node.isConnected) {
    throw new Error(
      `[context] findContextStore called on a detached <${
