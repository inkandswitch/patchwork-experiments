...
export const WeatherCardToken: ToolRender = (handle, element) => {
  ...
  const paint = () => {
    ...
    icon.textContent = (doc.emoji as string) || "⛅";
    ...
    const titleHint =
      typeof doc["@patchwork"]?.title === "string"
        ? placeFromTitle(doc["@patchwork"]?.title as string)
        : "";
    ...
    const hi =
      doc.tempMax == null ? "" : Math.round(doc.tempMax as number) + "°";
    const lo =
      doc.tempMin == null ? "" : Math.round(doc.tempMin as number) + "°";
    temp.textContent = hi && lo ? hi + " / " + lo : hi || lo || "…";

    chip.append(icon, lead, place, temp);
    if (doc.summary) chip.title = doc.summary as string;
    element.append(chip);
  };
