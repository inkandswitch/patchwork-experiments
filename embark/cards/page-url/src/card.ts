...
const card: ToolRender<PageUrlCardDoc> = (handle, element) => {
  ...
  const captureTab = (tab: ActiveTab) => {
...
function extensionApi(): PatchworkCardsApi | undefined {
  return (window as { patchworkCards?: PatchworkCardsApi }).patchworkCards;
}
...
function part(className: string, text: string): HTMLDivElement {
