...
import { runSchemaMatcher } from "@embark/schema";
...
// matcher engine against that canvas's context store: schemas published into
// `SchemaQueries` are matched over the documents in the `OpenDocuments`
// channel (fed by the Open Documents card and by cards minting synthetic
// docs), and match urls are answered into `SchemaMatches`. Flipping or
// removing the card releases the matches slice and stops answering. It renders
...
function SchemaMatcherCard(props: { element: ToolElement }) {
  onMount(() => {
    ...
    // resolves to the canvas's context (the page-global body store).
