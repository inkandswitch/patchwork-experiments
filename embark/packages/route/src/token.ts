...
export const RouteCardToken: ToolRender = (handle, element) => {
  ...
  const paint = () => {
    ...
    const ends =
      typeof doc["@patchwork"]?.title === "string"
        ? placesFromTitle(doc["@patchwork"]?.title as string)
        : { from: "", to: "" };
    ...
    icon.textContent = (doc.emoji as string) || "🗺️";
    ...
    if (doc.mode) chip.title = (doc.mode as string) + " route";
    element.append(chip);
  };

  // Wire a pill to its linked place doc: live title + click-to-open.
  function bindPlace(pill: HTMLSpanElement, placeUrl: unknown, myGen: number) {
    ...
    repo
      .find<Record<string, unknown>>(placeUrl as AutomergeUrl)
      .then((h) => {
        if (myGen !== gen) return;
        const paintTitle = () => {
...
function placesFromTitle(title: string): { from: string; to: string } {
...
function documentIdOf(url: string): string | null {
  try {
    return parseAutomergeUrl(url as Parameters<typeof parseAutomergeUrl>[0])
      .documentId;
  } catch {
    return null;
  }
