...
export type TextExtract = {
  text: string;
  // A DOM Range spanning the extracted `[from, to)` offsets, or null when the
  // offsets can't be mapped to live nodes.
...
export function extractText(root: Element): TextExtract {
  let text = "";
  // `boundaries[i]` is the DOM position at content offset `i` (for
  // `i` in `0..text.length`). Synthetic positions (block line breaks) are left
  // undefined and snapped to the nearest real boundary on lookup.
  const boundaries: (Pos | undefined)[] = [];
  ...
  const emit = (chunk: string, node: Text, nodeStart: number) => {
    flushBreak();
  ...
  const rangeFor = (from: number, to: number): Range | null => {
...
function clampOff(pos: Pos): number {
...
function isSkippedChrome(el: Element): boolean {
