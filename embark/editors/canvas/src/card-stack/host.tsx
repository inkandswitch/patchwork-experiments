...
// localStorage keys holding this browser's singletons: the global card stack
// (the "Global" tab) and the sidebar parts bin. Deliberately per-device, not
// synced through the account, so the same surfaces reopen across sessions.
const GLOBAL_STACK_URL_KEY = "embark:global-card-stack-url";
const PARTS_BIN_URL_KEY = "embark:parts-bin-url";
...
// The documents the sidebar needs, resolved once per host lifetime.
type CardsHostDocs = {
...
type CardsHostState = {
  host: ToolElement;
  // Set once the singletons resolve; used to remount after a fallback move.
  docs: CardsHostDocs | undefined;
...
function createCardsHostState(repo: Repo): CardsHostState {
  ...
  void resolveCardsHostDocs(repo).then(
    (docs) => {
      if (state.torndown) return;
      state.docs = docs;
  ...
  return render(
    () => (
      <RepoContext.Provider value={state.host.repo}>
        <CardsSidebar
          globalStack={docs.globalStack}
          partsBin={docs.partsBin}
...
// Find-or-create this browser's singletons: the global card stack and the
// sidebar parts bin (seeded with the standard examples). A stored url that
// can't be resolved in this repo (e.g. a different device or account) is
// treated as absent and a fresh doc is minted.
async function resolveCardsHostDocs(repo: Repo): Promise<CardsHostDocs> {
  const [globalStack, partsBin] = await Promise.all([
    findOrCreate<CardStackDoc>(repo, GLOBAL_STACK_URL_KEY, () =>
    ...
    findOrCreate<PartsBinDoc>(repo, PARTS_BIN_URL_KEY, () =>
      repo.create<PartsBinDoc>({
