...
type ContextStore = {
  read<T extends JSONObject>(channel: Channel<T>): T; // current merged value
  subscribe<T extends JSONObject>(
    channel: Channel<T>,
    cb: (value: T) => void,
  ): () => void;
  handle<T extends JSONObject>(channel: Channel<T>): ScopeHandle<T>;
};
```
...
// One-shot synchronous lookup: dispatch the request from `node` and read back
// whatever a host wrote into the event detail (undefined when none is found,
// so a tool opened outside a canvas degrades gracefully).
function findContextStore(node: Node): ContextStore | undefined;
...
gets `view.dom`, LLM cards get their host element. Because the request bubbles
and is `composed`, it reaches the host across sibling-embed and shadow
boundaries. Discovery is one-shot — once a consumer has the store it calls
`subscribe` / `handle` directly. The `<patchwork-context>` wraps the canvas's
content, so it always exists before its descendants mount and resolution is
synchronous.
...
- **Stable array order.** Concatenation order across scopes is unspecified —
  ...
  course preserved.)
- **Nested canvases.** A canvas embedded in a canvas is out of scope for now;
  assume one `<patchwork-context>` host per session. (The event-based discovery
  already degrades sensibly — the nearest host wins via `stopPropagation` — if
  this comes up later.)
