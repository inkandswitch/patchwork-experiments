...
You use:
  1. schema:queries / schema:matches  - find the map embed(s) on the canvas and read their live bounds.
...
## Discovery: find the map and read its bounds
...
The match urls are re-read as maps come and go - treat it as a live set. Usually there is one map; track the first. await repo.find(url) then read handle.doc().bounds, and listen on the handle's "change" to re-search when the box moves:
