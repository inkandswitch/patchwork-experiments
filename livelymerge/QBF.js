...
function findAllQBFScoresViewers() {
  /** Every open QBFScoresMorph (including under ephemeral panels). */
  let found = [];
  ...
  let visit = (m) => {
    ...
    seen.push(m);
    if (m.className === 'QBFScoresMorph') found.push(m);
    let kids = m.allSubmorphs ? m.allSubmorphs() : m.submorphs;
    if (kids) for (let i = 0; i < kids.length; i++) visit(kids[i]);
  };
  if (Lively.eachSubmorph) Lively.eachSubmorph(visit);
  else visit(Lively);
  return found;
}
function qbfRefreshAllScoreViewers() {
  /** Refresh every visible scores viewer (local post or remote merge). */
  qbfScoresNotify();
  let viewers = findAllQBFScoresViewers();
  for (let i = 0; i < viewers.length; i++) {
    try {
      if (viewers[i].refresh) viewers[i].refresh();
    } catch (err) {
      console.log('QBF scores viewer refresh error: ' + err);
    }
  }
}
...
function qbfPostLevelScore(playerName, levelCaption, record) {
  ...
  // Trim age / top-3-per-speed so the shared list stays display-ready.
  try {
    qbfPruneOldScores();
  } catch (err) {
    console.log('QBF prune after post error: ' + err);
  }
  qbfRefreshAllScoreViewers();
  return true;
}
...
function qbfHighScoreMaxAgeMs() {
  return Lively && Lively.$qbfHighScoreMaxAgeMs != null
    ? Lively.$qbfHighScoreMaxAgeMs
    : 30 * 24 * 60 * 60 * 1000;
}
function qbfHighScoreMaxPerSpeed() {
  return Lively && Lively.$qbfHighScoreMaxPerSpeed != null ? Lively.$qbfHighScoreMaxPerSpeed : 3;
}
function qbfEntryScoreValue(e) {
  if (!e) return 0;
  if (e.bestGame != null) return Number(e.bestGame);
  if (e.score != null) return Number(e.score);
  return 0;
}
function qbfFilterHighScoreEntries(entriesIfAny) {
  /**
   * Top-scores view / prune policy, per speed category:
   *   1. Group by speed (level)
   *   2. Drop entries older than the age limit (default 30 days; undated kept)
   *   3. Keep at most 3 highest scores in that category
   */
  let entries = entriesIfAny || [];
  let maxAge = qbfHighScoreMaxAgeMs();
  let maxPer = qbfHighScoreMaxPerSpeed();
  let bySpeed = {};
  for (let i = 0; i < entries.length; i++) {
    ...
    let speed = e.level != null ? e.level : e.speed != null ? e.speed : '';
    if (!bySpeed[speed]) bySpeed[speed] = [];
    bySpeed[speed].push(e);
  }
  let out = [];
  let speeds = Object.keys(bySpeed).sort((a, b) => qbfSpeedRank(a) - qbfSpeedRank(b));
  for (let s = 0; s < speeds.length; s++) {
    let list = bySpeed[speeds[s]];
    list.sort((a, b) => qbfEntryScoreValue(b) - qbfEntryScoreValue(a));
    for (let i = 0; i < list.length && i < maxPer; i++) out.push(list[i]);
  }
  return out;
}
function qbfPruneOldScores() {
  /**
   * Apply the high-score retention policy to the store (age + top 3 per speed).
   * Called when an epoch ends; also safe to call after posting.
   */
  let store = qbfScoresStore();
  if (!store || !store.getScoreEntries) return 0;
  let entries = store.getScoreEntries();
  if (!entries || entries.length === 0) return 0;
  let kept = qbfFilterHighScoreEntries(entries);
  let dropped = entries.length - kept.length;
  if (dropped === 0) {
    // Still may need to reorder / trim duplicates of equal length — only rewrite if changed.
...
class QBFScoresMorph extends Morph {
  ...
  showScoreEntries(entries) {
    let filtered = qbfFilterHighScoreEntries(entries || []);
    let scores = [];
  ...
  showScores(allScores) {
    ...
    let filtered = qbfFilterHighScoreEntries(flat);
    let scores = [];
    for (let i = 0; i < filtered.length; i++) {
      let e = filtered[i];
      let gs = QBFGameScore.fromAny(e);
  ...
  renderScoreLines(scores) {
    scores.sort((a, b) => {
      let ra = qbfSpeedRank(a.speed);
      let rb = qbfSpeedRank(b.speed);
      if (ra !== rb) return ra - rb;
    ...
    let footer =
      '\n      -- Top 3 per speed, retained ' +
      Math.round(qbfHighScoreMaxAgeMs() / (24 * 60 * 60 * 1000)) +
      ' days --';
    if (scores.length === 0) {
      this.setScoreListText(
        this.scoresScroll,
...
function qbfEnsureWorldMenuItem(menuIfAny, optsIfAny) {
  /**
   * Install (or repair) the world-menu entry that opens QBF.
   * Rebind $menuActions always (replica-local). addItemBefore writes Automerge —
   * skip it when opts.actionsOnly (merge rebind); only add the label from init/load.
   */
  if (typeof openQBF !== 'function') return null;
  let opts = optsIfAny || {};
  let wm = menuIfAny;
  if (!wm && typeof findPersistentWorldMenu === 'function') wm = findPersistentWorldMenu();
  if (!wm && typeof Lively !== 'undefined' && Lively && Lively.findA)
    wm = Lively.findA('MenuMorph');
  if (!wm || !wm.findItem || !wm.addItemBefore) return null;
  let idx = wm.findItem('Quick Brown Fox');
  if (idx < 0) idx = wm.findItem('brown fox');
  // Resolve openQBF at click time (not via a stale arrow closure).
  let open = function () {
    openQBF();
  };
  if (idx < 0) {
    if (opts.actionsOnly) return wm;
    wm.addItemBefore('transcript', 'Quick Brown Fox', open);
    return wm;
  }
  wm.ensureActionList && wm.ensureActionList();
  if (wm.$menuActions) wm.$menuActions[idx] = open;
  return wm;
}
if (typeof Lively !== 'undefined' && Lively && Lively.addEphemeralMorph) {
  qbfEnsureWorldMenuItem();
}
