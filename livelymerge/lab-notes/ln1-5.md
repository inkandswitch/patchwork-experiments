...
Automerge's promise is _convergence_: after two clients exchange their changes, they are guaranteed to arrive at the same state. Note what's missing from that sentence: nothing says the state they agree on is one your program can live with.
...
![image](lm-linked-list-problem.png)
...
You can dodge this particular example by not building linked lists out of `next` pointers. Automerge has built-in datatypes with well-behaved merge semantics — its arrays (which our object model exposes directly) merge concurrent insertions and deletions the way you'd hope, and maps of various flavors are easy to represent. We lean on this hard in practice: Morphic's `submorphs` are Automerge arrays, and concurrent adds from two users interleave just fine.
...
In a system like LM — where the heap **is** the document, and users are encouraged to build whatever data structures they like — this isn't a corner case. It's the central open problem, and we're actively thinking about it.

## Directions we're considering

None of these are implemented; I'm just thinking out loud here.

**Make the invariant unrepresentable to violate.** Often the invariant exists because the representation is redundant: `next` and `prev` say the same thing twice; `owner` and `submorphs` do too. Store one direction and _derive_ the other, and there's nothing left to disagree. Better still, choose a representation whose built-in merge semantics already coincide with the invariant — a "linked list" stored as an Automerge array can't have a cycle, by construction. This is less a mechanism than a design discipline, and it doesn't cover everything, but it's striking how far it goes.

**Merge-aware datatypes.** Today, merging happens at the level of the raw object graph, below the abstractions the programmer actually cares about. Given the diagnosis above, the natural move is to merge the _intents_ instead of the writes they compiled down to. And notice that the intent isn't "set `3.next` to null" — it isn't even "swap 3 and 4", which is still implementation, just one level up. The intent is what the programmer would say at the level of the abstract type: "remove this value from the list", "insert this value after that one".
...
**An invariant-repair pass at the merge boundary.** Let the programmer declare invariants (or write repair functions), and run them whenever remote changes arrive. Conveniently, LM already stops the world at every transaction boundary for GC and promotion, so there's a natural place to hang this. And there's a pleasant property lurking here: if repair is a deterministic function of the merged state, every replica repairs identically, and convergence is preserved. (Repairing _well_ — without discarding someone's work — is the hard part.)

**Surface it to people.** When all else fails, treat a violated invariant the way git treats a conflicting hunk: detect it, quarantine it, and ask a human. It's the least automatic option, but it's honest — and in a system whose whole point is that people can see and manipulate everything, "show the user the broken list and let them fix it" might be less crazy than it sounds.

If you've thought about this problem — or you're sitting on a fifth direction — let's talk!
