...
const morph = {
  ...
  submorphs: [a, b],       // persistent edge
  $submorphs: [haloMorph], // local edge — never shared
};
```
...
Rendering and hit-testing treat the two lists uniformly: a morph's submorphs — persistent and ephemeral alike — are merged into a single stacking order, with per-user UI like halos kept in an always-on-top layer. And here's the economy of the edges-not-objects rule: only the _attachment_ is local. The halo's own subtree — its handles, their shapes, their labels — is made of perfectly ordinary objects, connected by perfectly ordinary properties. They stay local anyway, because the only way to reach them is through that one `$`-edge.
