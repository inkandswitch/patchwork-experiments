...
**A word about who this note is for.** Our use of Automerge is deliberately extreme — a stress test — so every cost below also exists in ordinary Automerge apps, just not to the same degree. If your app is slow, a gentler version of one of these might be why. And if you only have time for one section, skip ahead to Optimization 4, "minimizing the cost of strings": it doesn't depend on the others, it applies to every app that stores strings, and it's how our documents came to have 88× fewer operations and load 14× faster.
