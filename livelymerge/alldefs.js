...
w.debugReparentCheck = function (morph, label) {
  ...
  let world = morph.world && morph.world();
  if (world) scan(world, 'world');
  let ownerName = morph.owner ? morph.owner.className || 'Morph' : 'null';
  let ownerIx = morph.owner && morph.owner.submorphs ? morph.owner.submorphs.indexOf(morph) : -1;
  let worldIx = world && world.submorphs ? world.submorphs.indexOf(morph) : -1;
  console.log(
    '[reparent]',
    label || '',
    'id',
