...
const TitlebarToolComponent: React.FC<{ element: HTMLElement, docUrl: AutomergeUrl }> = ({ element, docUrl }) => {
  const [doc] = useDocument<TaskQueue>(docUrl, { suspense: true });

  return (
    <div className="h-full flex items-center" onClick={() => element.dispatchEvent(createOpenEvent({ url: docUrl }))}>
      <span>{doc.title ?? 'TQ'}</span>
      <span>{doc.pending.length}</span>
      /
      <span>{doc.done.length}</span>
    </div>
  );
};
...
export const TitlebarTool = (handle: DocHandle<unknown>, element: HTMLElement) => {
  ...
  root.render(
    <RepoContext.Provider value={repo}>
      <TitlebarToolComponent element={element} docUrl={'automerge:38YYaP6izqpmgUcTK2NNhmDJj1fD' as AutomergeUrl} />
