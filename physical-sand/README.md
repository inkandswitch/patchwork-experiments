...
A falling-sand tool for [Physical Patchwork](../physical-frame). Sand rains from the top of the box and collides with the **marks** detected by the [marks layer](../physical-layer-marks/) (black marker drawings or black objects on the surface), treating them as physical barriers.
...
- Inlines the dependency-free `subscribe` helper and subscribes to `physical:marks`
...
2. In a Physical Patchwork host (walls layer active, background sampled), Use mode → **Create new ▾** →
   Physical Sand; set it active and start the camera.
3. Press **Start**, then draw on the surface and watch the sand react.

## Tunables (top of `physical-sand.js`)
