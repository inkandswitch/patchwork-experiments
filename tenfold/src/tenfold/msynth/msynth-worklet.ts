...
import type { MessageFromWorklet, MessageToWorklet } from "./types.ts";
  ...
  implements AudioWorkletProcessorImpl
{
  ...
  constructor() {
    super();
    this.port.onmessage = (msg: MessageEvent<MessageToWorklet>) =>
      ...
      case "load patch": {
        try {
          ...
          this.synth = new Synth(
            outs,
            allSignals,
            new Float32Array<any>(msg.params)
          );
        } catch (e: any) {
          this.sendMessage({
            event: "log",
            message: `error loading patch: ${e.message}`,
          });
          console.error(e);
        }
        break;
