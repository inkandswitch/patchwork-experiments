...
import { SAMPLE_RATE, TAU } from "./constants.ts"
import { Signal } from "./signal.ts"
import { clamp, fastTanh } from "./helpers.ts"
...
export function ladder(mode: "lp24" | "lp12" | "bp24" | "bp12" | "hp24" | "hp12", _input = Signal.scalar(0), _cutoffFreq = Signal.scalar(20000), _q = Signal.scalar(0.2)) {
  let z0 = [0, 0, 0, 0]
  let z1 = [0, 0, 0, 0]
  let alpha = 1
  let k = 1
  ...
  function updateFreq() {
    ...
    const wc = clamp(_cutoffFreq.value, MIN_CUTOFF_FREQ, MAX_CUTOFF_FREQ) * TAU * SAMPLE_RATE_INTERPOLATION_RECIP
    const wc2 = wc * wc
    const wc3 = wc * wc2
    const wc4 = wc * wc3
  ...
  function weightedSumForCurrentMode(stageOuts0: number, stageOuts1: number, stageOuts2: number, stageOuts3: number, stageOuts4: number) {
    switch (mode) {
      case "lp24":
        return stageOuts4
      case "lp12":
