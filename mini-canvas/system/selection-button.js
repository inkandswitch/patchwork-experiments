...
export const schema = {
  init() {
    return { x: 40, y: 10, toolUrl: new URL('./selection-button.js', import.meta.url).href };
  },
  parse(value) {
...
export default function mount(element) {
  ...
  function onKeyDown(event) {
    ...
    console.log('[selection] deleting shapes', ids);
    console.log('[selection] shapes before delete', JSON.parse(JSON.stringify(canvas.ref.at('shapes').value())));
    canvas.ref.at('shapes').change((shapes) => {
      for (const id of ids) {
        console.log('[selection] deleting', id, 'exists:', id in shapes);
        delete shapes[id];
      }
    });
    console.log('[selection] shapes after delete', JSON.parse(JSON.stringify(canvas.ref.at('shapes').value())));
    selectedShapesRef.change(() => ({}));
    console.log('[selection] selectedShapes cleared');
  }
