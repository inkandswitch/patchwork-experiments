/**
 * Physical Colors — a bundleless demo tool for the physical host.
 *
 * Subscribes to the host's `physical:apriltags` provider. Colors are hardcoded by
 ...
 *
 * @typedef {Object} PhysicalColorsDoc
...
// Datatype
// ---------------------------------------------------------------------------
export const PhysicalColorsDatatype = {
  init(doc) {
    doc.title = "Physical Colors";
  },
  getTitle(doc) {
    return doc.title || "Physical Colors";
  },
  setTitle(doc, title) {
    doc.title = title;
  },
...
export function Tool(handle, element) {
  ...
  style.textContent = `
    ...
    .physical-colors canvas.sc-field {
      position: absolute;
      inset: 0;
      width: 100%;
      height: 100%;
    ...
    .physical-colors polygon.sc-outline {
      fill: none;
  ...
  root.className = "physical-colors";
  ...
  const unsubCoords = subscribe(
    element,
    { type: "physical:coordinate-system" },
    (value) => {
...
export const plugins = [
  {
    ...
    id: "physical-colors",
    name: "Physical Colors",
    icon: "Palette",
    async load() {
      return PhysicalColorsDatatype;
    },
  },
  {
    type: "patchwork:tool",
    ...
    name: "Physical Colors",
    icon: "Palette",
    supportedDatatypes: ["physical-colors"],
    async load() {
      return Tool;
    },
  },
