...
import { runLLMProcess, type RunResult } from '../../process/llm-process';
import { ProcessView } from '../../process/components/ProcessView.tsx';
import { WorkspaceUI } from '../../workspace/components/WorkspaceUI.tsx';
import type { WorkspaceChange, WorkspaceChanges } from '../../workspace/types';
import type { ProcessDoc } from '../../process/types';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        {activeTab === 'worker' && (
          <WorkerTab workerDocUrl={workerDocUrl} />
        )}
        {activeTab === 'workspace' && doc?.workspaceUrl && (
          <WorkspaceUI docUrl={doc.workspaceUrl} />
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '8px' }}>
        ...
        {processUrls.length === 0 && (
          <div style={{ textAlign: 'center', color: '#bbb', fontSize: 12, padding: 24 }}>
            No runs yet
          </div>
        )}

        {/* Previous runs (collapsed by default) */}
        {processUrls.slice(0, -1).reverse().map((url, reverseIdx) => {
          ...
          return (
            <div key={url} style={{ marginBottom: 4 }}>
              ...
              {isExpanded && (
                <ProcessView processUrl={url as AutomergeUrl} />
              )}
            </div>
          );
        })}
        ...
        {processUrls.length > 0 && (
          <div>
            <div style={{ fontSize: 11, color: '#888', padding: '4px 0', fontWeight: 600 }}>
              Run {processUrls.length} (latest)
            </div>
            <ProcessView
              processUrl={processUrls[lastIdx] as AutomergeUrl}
              isActive={isRunning}
            />
          </div>
        )}
