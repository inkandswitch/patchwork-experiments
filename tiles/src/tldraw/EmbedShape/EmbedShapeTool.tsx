 ...
 */
function getDefaultToolId(datatypeId: string): string {
...
export class EmbedShapeTool extends StateNode {
  ...
  override onPointerUp(_info: TLPointerEventInfo) {
    ...
    if (!ctx) {
      console.warn('[tiles] EmbedShapeTool: context not set for this editor');
      this.editor.setCurrentTool('select');
      return;
    }
    ...
    if (!datatypeId) {
      console.warn('[tiles] EmbedShapeTool: no datatype selected');
      this.editor.setCurrentTool('select');
      return;
    }
    ...
    (async () => {
      try {
        ...
        console.error('[tiles] EmbedShapeTool: doc creation failed:', err);
        if (editor.getShape(placeholderId)) {
          editor.updateShape({
            id: placeholderId,
            type: EMBED_SHAPE_TYPE,
