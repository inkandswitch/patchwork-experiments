...
import { buildHistory } from '../serialize-process';
...
function ChatTab({
  ...
  const handleRun = useCallback(async () => {
    ...
    const history = processUrls.length > 0
      ? await buildHistory(repo, processUrls as AutomergeUrl[])
      : undefined;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: '8px' }}>
        {processUrls.map((url, idx) => (
          <ProcessView
            key={url}
            processUrl={url as AutomergeUrl}
            isActive={idx === processUrls.length - 1 && isRunning}
          />
        ))}
