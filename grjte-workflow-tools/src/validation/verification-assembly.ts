...
import type { SpecDoc, VerificationDoc } from "../workflow-types";
...
export async function loadSpecTree(
  ...
  const verifications = await Promise.all(
    (doc.spec.verificationUrls ?? []).map(async (verificationUrl) => {
      ...
      if (verification.docUrl) {
        const datalogHandle = (await repo.find(
          verification.docUrl,
        )) as RepoDocHandle<DatalogDoc>;
        return {
          url: verificationUrl,
  ...
  return {
    ...
    verifications: verifications.filter(
      (entry): entry is NonNullable<(typeof verifications)[number]> =>
        entry !== null,
    ),
    dataDocs,
    subSpecs: subSpecs.filter(
      (entry): entry is NonNullable<(typeof subSpecs)[number]> =>
        entry !== null,
    ),
  };
}

export function flattenSpecTree(
  node: SpecTreeNode | null | undefined,
  ...
  return [
    ...node.verifications.map((verification) => ({
      ...
      nodeGoal: node.goal,
      targetKind:
        node.path === "root" ? ("global" as const) : ("scoped" as const),
      verification,
...
async function loadDataDocsFromFolder(
  ...
  const nestedDocs = await Promise.all(
    folder.docs.map(async (entry) => {
      ...
      const dataHandle = (await repo.find(
        entry.url,
      )) as RepoDocHandle<DatalogDoc>;
      return [
        {
...
export async function watchSpecTree(
  ...
  async function rebuild() {
    ...
    async function loadWatchedDataDocs(
      folderUrl: AutomergeUrl,
    ): Promise<LoadedDataDoc[]> {
      const folderHandle = (await repo.find(
        folderUrl,
      )) as RepoDocHandle<FolderDoc>;
          ...
          return [
            {
              url: entry.url,
              name:
                entry.name || dataHandle.doc()?.title || "Untitled data doc",
              title: dataHandle.doc()?.title,
              datalogDoc: dataHandle.doc(),
            } satisfies LoadedDataDoc,
          ];
