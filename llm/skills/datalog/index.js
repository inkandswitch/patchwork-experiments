...
export class DocDatalog extends Datalog {
  ...
  async _refresh() {
    this._doc = await this._handle.doc();
    return this._doc;
  }
  ...
  get facts() {
    return this._doc?.facts ?? [];
  }

  get rules() {
    return this._doc?.rules ?? [];
  }

  get constraints() {
    return this._doc?.constraints ?? [];
  }
  ...
  async assertFact(pred, args, comment) {
    await this._refresh();
  ...
  async assertConstraint(name, constraint) {
    await this._refresh();
  ...
  async query(pred) {
    await this._refresh();
    return super.query(pred);
  }

  async checkConflicts(constraintName) {
    await this._refresh();
    return super.checkConflicts(constraintName);
  }
}
...
export function createDatalog(repo, title) {
  ...
  return new DocDatalog(handle, null);
}
...
export async function getDatalog(repo, url) {
  ...
  return new DocDatalog(handle, doc);
}
 ...
 */
export async function mergeDatalog(repo, urls) {
  const facts = [];
  const rules = [];
  const constraints = [];
