...
export function PandocEditor(props: {
  ...
  // ─── engine ───
  const [engine, setEngine] = createSignal<"loading" | "ready" | "error">(
    "loading",
  );
  ...
  function mainIndex(d: PandocDoc): number {
    ...
    if (typeof main === "number" && main >= 0 && main < inputs.length)
      return main;
  ...
  async function addPatchworkDocs(
    items: { url: AutomergeUrl; name?: string }[],
  ) {
  ...
  function safeTitle(title: string): string {
    return (
      title.replace(/[\\/:]+/g, "-").replace(/\.[A-Za-z0-9]+$/, "") ||
      "document"
    );
  }
  ...
  const [pasteName, setPasteName] = createSignal("pasted.md");
  let pasteArea: HTMLTextAreaElement | undefined;

  function openPaste() {
    setPasteText("");
    setPasteName(uniqueInputName("pasted.md"));
    setPasteOpen(true);
    queueMicrotask(() => pasteArea?.focus());
  }

  /** Make sure a pasted file's name doesn't collide with an existing input. */
  function uniqueInputName(name: string): string {
    const existing = new Set(inputs().map((i) => i.name));
  ...
  function addPastedText() {
    ...
    const name = uniqueInputName(pasteName().trim() || "pasted.md");
    const url = createTextFileDoc(repo, name, text);
    addInputs([{ name, url }]);
    setPasteOpen(false);
  }
  ...
  return (
    ...
    >
        ...
        <aside class="pandoc-inputs">
          <div class="pandoc-inputs-header">
            <div class="pandoc-icon-btns">
              <button class="pandoc-btn small" onClick={openPaste}>
                + Paste
              </button>
              <button
      ...
      <Show when={pasteOpen()}>
        <div
          class="pandoc-modal-backdrop"
          onClick={(e) => {
            if (e.target === e.currentTarget) setPasteOpen(false);
          }}
        >
          <div class="pandoc-modal pandoc-paste-modal">
            <div class="pandoc-modal-header">
              <div>
                <h3>Paste text</h3>
                <p>Add typed or pasted text as an input.</p>
              </div>
              <button
                class="pandoc-modal-close"
                title="Close"
            ...
            <div class="pandoc-modal-body pandoc-paste-body">
              <textarea
                ref={pasteArea}
                class="pandoc-paste-area"
            ...
            <div class="pandoc-modal-footer pandoc-paste-footer">
              <label class="pandoc-paste-name">
                <span>Name</span>
                <input
                  type="text"
                  value={pasteName()}
              ...
              <button
                class="pandoc-btn small primary"
