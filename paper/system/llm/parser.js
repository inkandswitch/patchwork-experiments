/**
 * Streaming parser: extracts <script> blocks from an async token stream.
 * @param {AsyncIterable<string>} stream
 * @returns {AsyncGenerator<{ id: number, type: string, content?: string, code?: string, description?: string, complete: boolean }>}
 */
export async function* parseScriptBlocks(stream) {
  let buffer = '';
  let state = 'text';
  let scriptBuffer = '';
  let blockId = 0;
