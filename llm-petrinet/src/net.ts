...
export async function createDocumentCopy(repo: Repo, sourceHandle: DocHandle<Record<string, unknown>>): Promise<DocHandle<Record<string, unknown>>> {
...
export const PETRINET_SYSTEM_PROMPT = ["You are a coding agent that can execute JavaScript to accomplish tasks.", "", "Execute code by writing it inside <script> tags with a data-description attribute:", "", '<script data-description="Brief description of what this code does">', "// your code here", "</script>", "", "Rules:", "- Write one <script> block per iteration; wait for its output before continuing.", "- Use `return` to inspect values and `console.log` for intermediate output.", "", "Working with Automerge documents:", "- `repo.find(url)` is async — always `await` it", "- Read a document with `await handle.doc()`", "- Mutate documents with `handle.change()`", "- Use `updateText` from `@automerge/automerge-repo` for text content mutations"].join("\n");

// ─── Default system prompt templates ──────────────────────────────────────────

export const DEFAULT_OPTIMIZER_SYSTEM_PROMPT = ["Task: $PROMPT", "", "Document: $DOC_URL", "", "Step 1: Read the document.", "```", 'const handle = await repo.find("$DOC_URL")', "const doc = await handle.doc()", "return doc.content", "```", "", "Step 2: Write your solution to the document using the raw Automerge API:", "```", 'const { updateText } = await import("@automerge/automerge-repo")', 'const handle = await repo.find("$DOC_URL")', "const doc = await handle.doc()", 'handle.change(d => updateText(d, ["content"], "your improved content here"))', "```"].join("\n");

export const DEFAULT_EVALUATOR_SYSTEM_PROMPT = ["Criteria: $PROMPT", "", "Step 1: Read all solution documents.", "```", "const urls = $SOLUTION_URLS", "const reads = await Promise.all(urls.map(url =>", '  repo.find(url).then(h => h.doc()).then(d => ({ url, content: d?.content ?? "" }))', "))", 'return reads.map(r => `--- ${r.url} ---\\n${r.content}`).join("\\n\\n")', "```", "", "Step 2: Pick the best version based on the criteria above.", "Respond with ONLY the URL of the winning solution — a single line, nothing else."].join("\n");
    ...
    transitions: [
      {
        ...
        async onConsumedTokens({ problems }, { optimizer_idle }, repo) {
          ...
          const systemTemplate = systemPromptUrl ? await readDocContent(repo, systemPromptUrl) : (doc?.systemPrompts?.optimizer ?? DEFAULT_OPTIMIZER_SYSTEM_PROMPT);
          const produce = [];
        ...
        onProducedToken(token, _handle, repo) {
          if (token.placeId === "optimizer_running") {
            runLLMProcessRaw(repo, token.state.documentUrl as unknown as AutomergeUrl).catch((e) => console.error("[llm-petrinet] optimizer runLLMProcess error", e));
          }
        },
      },

      {
        ...
        async onConsumedTokens({ optimizer_running }) {
          return {
            produce: [{ state: { type: "optimizer", documentUrl: optimizer_running.state.optimizerUrl }, toPlace: "optimizer_idle" }],
          };
        },
      },

      {
        ...
        async onConsumedTokens({ evaluator_idle }, { solutions }, repo) {
          ...
          const systemTemplate = systemPromptUrl ? await readDocContent(repo, systemPromptUrl) : (doc?.systemPrompts?.evaluator ?? DEFAULT_EVALUATOR_SYSTEM_PROMPT);
        ...
        onProducedToken(token, _handle, repo) {
          if (token.placeId === "evaluator_running") {
            runLLMProcessRaw(repo, token.state.documentUrl as unknown as AutomergeUrl).catch((e) => console.error("[llm-petrinet] evaluator runLLMProcess error", e));
          }
        },
      },

      {
        ...
        async onConsumedTokens({ evaluator_running }, _allTokens, repo) {
          ...
          return {
            produce: [{ state: { type: "evaluator", documentUrl: evaluator_running.state.evaluatorUrl }, toPlace: "evaluator_idle" }, { state: { type: "problem", documentUrl: winnerUrl }, toPlace: "problems" }, ...solutionUrls.filter((url) => url !== winnerUrl).map((url) => ({ state: { type: "solution", documentUrl: url }, toPlace: "solutions" }))],
          };
        },
      },
    ],
...
async function launchOptimizerRun(repo: Repo, sourceHandle: DocHandle<Record<string, unknown>>, charPrompt: string, systemTemplate: string): Promise<{ copyUrl: string; processUrl: string }> {
...
async function launchEvaluatorRun(repo: Repo, solutions: ReadonlyToken[], evaluatorPrompt: string, systemTemplate: string): Promise<{ processUrl: string }> {
...
function buildOptimizerPrompt(systemPrompt: string, characterPrompt: string, docUrl: string): string {
  return systemPrompt.replace(/\$PROMPT/g, characterPrompt).replace(/\$DOC_URL/g, docUrl);
}

function buildEvaluatorPrompt(systemPrompt: string, evaluatorPrompt: string, solutionUrls: string[]): string {
  const urlsArray = JSON.stringify(solutionUrls, null, 2);
  return systemPrompt.replace(/\$PROMPT/g, evaluatorPrompt).replace(/\$SOLUTION_URLS/g, urlsArray);
}
