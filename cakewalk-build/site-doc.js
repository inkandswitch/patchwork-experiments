...
/**
 * A URL without its heads.
 *
 * Marking `public/**` an artifact makes pushwork pin the folder links leading to it, and a
 * heads-pinned URL resolves to a *view-only* handle: reading works, `change()` throws. It would
 * also be wrong to leave pinned even if it were writable — a pinned link freezes everything
 * below it, so a rebuilt page would never be seen through it. Build output is rewritten
 * continuously, so its links are written bare and stay current. The content is still an
 * ImmutableString, which is the part that matters: no text CRDT for generated bytes.
 */
