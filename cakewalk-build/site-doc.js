...
export async function writeSiteInto(repo, rootUrl, { entries, prefix = OUTPUT_PREFIX, shape }) {
  ...
  const folderAt = async (handle, name) => {
    ...
    if (link?.url) {
      ...
      if (child && Array.isArray(child.doc()?.docs)) {
        // An existing folder reached through a pinned link: keep the folder, drop the pin, so
        // what is written into it can be seen through its parent.
        if (String(link.url) !== bare(link.url)) {
          handle.change((d) => {
            const at = d.docs.findIndex((l) => l?.name === name);
            if (at >= 0) d.docs[at] = { name, type: "folder", url: bare(link.url) };
          });
        }
        return child;
      }
  ...
  for (const [dir, files] of byDir) {
    ...
    for (const [path, entry] of files) {
      ...
      const at = existing.get(name);
      links.push({ name, type: linkType(name), url: bare(await putFile(path, entry, at ? bare(at) : undefined)) });
    }
    ...
    handle.change((d) => {
      ...
      const folders = current
        ...
        .map((l) => ({ name: l.name, type: l.type, url: bare(l.url) }));
