...
export function Tool(handle, element) {
  ...
  async function ensureDetector() {
    ...
    try {
      ...
      // NOTE: physical-frame does NOT run the AprilTag detector itself — tag
      // reading lives in the physical-layer-apriltags package. This legacy
      // detector path is unused here; the @vite-ignore + .href keeps Vite from
      // trying to resolve the (absent) vendored worker at build time.
      detectorWorker = new Worker(
        /* @vite-ignore */ new URL("../vendor/apriltag.js", import.meta.url).href,
      );
