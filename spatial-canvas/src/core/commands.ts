...
export function translateShapes(handle: DocHandle<CanvasDoc>, moves: Map<string, { x: number; y: number }>): void {
  handle.change((doc) => {
...
export function resizeShape(handle: DocHandle<CanvasDoc>, id: string, next: { x: number; y: number; width: number; height: number }): void {
  handle.change((doc) => {
    if (doc.shapes[id]) {
      doc.shapes[id].x = next.x;
      doc.shapes[id].y = next.y;
      doc.shapes[id].width = next.width;
      doc.shapes[id].height = next.height;
    }
  });
}
