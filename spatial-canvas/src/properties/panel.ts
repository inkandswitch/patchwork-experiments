...
export default function PropertiesPanel(
  ...
  for (const color of COLORS) {
    ...
    wrap.style.cssText =
      "display:flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:5px;transition:background 0.1s;";
  ...
  function getCurrentFill(): RectangleFill {
    return (
      (handle.doc()?.stateByUser?.[contactUrl]?.fill as RectangleFill | undefined) ?? DEFAULT_FILL
    );
  }
  ...
  for (const mode of FILL_MODES) {
    const btn = document.createElement("button");
    btn.style.cssText =
      "width:32px;height:32px;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;border-radius:5px;transition:background 0.1s;";
  ...
  for (const { label, value } of FONT_SIZES) {
    const btn = document.createElement("button");
    btn.style.cssText =
      "width:32px;height:32px;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;border-radius:5px;transition:background 0.1s;font:13px/1 system-ui,sans-serif;font-weight:500;color:#444;";
