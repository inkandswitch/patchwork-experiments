...
function createScriptBlockEl(block: Extract<OutputBlock, { type: 'script' }>): HTMLElement {
  ...
  header.style.cssText = [
    ...
    'padding:2px 0', 'font:11px/1 system-ui,sans-serif', 'color:#888',
  ].join(';')
  ...
  if (block.output || block.error) {
    ...
    none.style.cssText = 'font:10px/1 system-ui,sans-serif;color:#ccc;font-style:italic;margin-top:2px;'
...
export default function ProcessViewer(
  ...
  element.style.cssText = [
    ...
    'padding:6px 0', 'font-family:system-ui,sans-serif', 'font-size:12px',
  ].join(';')
    ...
    if (blocks.length > lastBlockCount) {
      for (let i = lastBlockCount; i < blocks.length; i++) {
        ...
        if (block.type === 'text') {
          ...
          el.style.cssText = 'font-size:12px;color:#444;line-height:1.5;white-space:pre-wrap;word-break:break-word;'
