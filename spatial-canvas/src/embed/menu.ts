...
export function openMenu(
  ...
  for (const item of items) {
    ...
    const iconData = item.icon
      ...
      : undefined
    if (iconData) {
      row.appendChild(createElement(iconData, { width: 16, height: 16, style: 'pointer-events:none;flex-shrink:0' }))
    }

    const label = document.createElement('span')
    label.textContent = item.name
    label.style.cssText = 'flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;pointer-events:none;'
