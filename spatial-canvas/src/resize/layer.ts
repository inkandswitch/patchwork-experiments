...
const HANDLES: HandleDef[] = [
  // Corners — H×H squares, flush against the outside corners
  {
    type: "nw",
    cursor: "nwse-resize",
    style: `top:-${H}px;left:-${H}px;width:${H}px;height:${H}px;`,
  },
  {
    type: "ne",
    cursor: "nesw-resize",
    style: `top:-${H}px;right:-${H}px;width:${H}px;height:${H}px;`,
  },
  {
    type: "se",
    cursor: "nwse-resize",
    style: `bottom:-${H}px;right:-${H}px;width:${H}px;height:${H}px;`,
  },
  {
    type: "sw",
    cursor: "nesw-resize",
    style: `bottom:-${H}px;left:-${H}px;width:${H}px;height:${H}px;`,
  },
...
export default function ResizeLayer(
  ...
  function onPointerMove(e: PointerEvent) {
    ...
    const canvas = drag.handleEl.closest<SpatialCanvasHost>(
  ...
  function buildWrapper(shapeId: string): HTMLElement {
    ...
    for (const def of HANDLES) {
      ...
      h.addEventListener("pointerdown", (e: PointerEvent) => {
        ...
        const canvas = h.closest<SpatialCanvasHost>(
          'patchwork-view[tool-id="spatial-canvas"]',
        )?.spatialCanvas;
        if (!canvas) return;
