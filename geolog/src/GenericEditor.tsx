...
import { GeologAutomerge, type DerivedSort, type SignatureInfo } from './geolog-automerge';
  ...
  return (
    <section className="generic-sort-section">
      ...
      <div className="entity-list">
        ...
        {entities.length === 0 && <span className="empty">No entities yet</span>}
      </div>
  ...
  return (
    <section className="generic-rel-section">
      ...
      <div className="tuple-list">
        ...
        {tuples.length === 0 && <span className="empty">No tuples yet</span>}
      </div>
      ...
      {canShowForm ? (
        ...
        <p className="hint">Add entities for all referenced sorts to create {relName} tuples</p>
      )}
    </section>
  );
}
...
/**
 * Generic database editor driven by the theory signature.
...
export function GenericEditor({ geolog }: GenericEditorProps) {
  ...
  const sortIdToName = useMemo(() => buildSortIdToName(exported.signature), [exported]);
  ...
  const addRelation = useCallback(
    (relName: string, args: Array<{ entity: string } | { int: number } | { str: string }>) => {
      setError(null);
