...
export const TinyTool = (handle: DocHandle<unknown>, element: HTMLElement) => {
  const dispose = render(
    () => <RawEditor handle={handle} element={element} />,
    element
  );
  return dispose;
};
...
function RawEditor(props: {
  ...
  const initialDoc = props.handle.isReady()
    ...
    : undefined;
  const [doc, setDoc] = createSignal<Record<string, unknown> | undefined>(
    initialDoc
  );
  ...
  if (!initialDoc) {
    props.handle
      .whenReady()
      .then(() => setDoc(props.handle.doc() as Record<string, unknown>));
  }
  ...
  return (
    ...
    >
      <Show
        when={doc()}
        fallback={<div class="re-loading">Loading {docUrl}…</div>}
      >
        {(loadedDoc) => (
          <>
