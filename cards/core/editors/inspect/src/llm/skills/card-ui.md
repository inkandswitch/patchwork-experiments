...
of the `highlight` channel (owned by `@embark/selection`) and clear-and-set
it on hover:
...
const highlight = getContextHandle(element, Highlight);
...
(Declare `@embark/selection` — `automerge:3FqZv79rgfNX5nKn9kkpWGCSQUjW` — in
