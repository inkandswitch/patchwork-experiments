// The inline token face for a poi-card: a compact chip reading "📍 <name>",
...
// signature is the standard tool contract that `<patchwork-view>` (and the
// shared `token-view` that delegates to it) call.

export const PoiCardToken = (handle, element) => {
  const paint = () => {
    ...
    const title =
      ...
      (typeof doc["@patchwork"]?.title === "string" && doc["@patchwork"]?.title) ||
      "Place";
