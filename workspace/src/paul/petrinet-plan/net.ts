...
export const PETRINET_SYSTEM_PROMPT = [
  ...
  '- ALWAYS read the full content of every document before acting on it.',
  ...
  '  If a document references other documents (e.g. via URLs in its fields), follow those references and read them too.',
  '',
...
export const DEFAULT_OPTIMIZER_SYSTEM_PROMPT = [
  ...
  '</script>',
  '',
  'Step 2: Read any verification docs referenced by the spec. Follow all document references.',
  ...
  '</script>',
  '',
  'Step 3: Read the FULL content of every document you will modify. Inspect all fields.',
  '<script data-description="Read current documents">',
  'const docs = $DOC_URLS',
  'const results = await Promise.all(docs.map(async ({ name, url }) => {',
  '  const h = await repo.find(url)',
  '  const d = await h.doc()',
  '  return { name, url, content: d }',
  '}))',
  'return JSON.stringify(results, null, 2)',
  '</script>',
  '',
