...
export function serializeConstraint(constraint: StoredConstraint): string {
  ...
  return constraint.comment !== undefined ? `// ${constraint.comment}\n${line}` : line;
}
...
function consumeStatements(buffer: string): { statements: string[]; remainder: string } {
...
function parseAtom(source: string): StoredAtom {
  ...
  const args = inner ? splitTopLevel(inner, ',').map((entry) => entry.trim()) : [];
  return { pred, args };
}
...
function parseConstraintStatement(source: string, comment?: string): StoredConstraint {
...
export function parseProgram(source: string): {
  ...
  const flushStatements = () => {
    ...
    for (const statement of statements) {
      ...
      if (!trimmed) continue;
      const comment = pendingComments.length > 0 ? pendingComments.join('\n') : undefined;
      pendingComments = [];
  ...
  for (const line of lines) {
    ...
    if (!buffer.trim() && (trimmed.startsWith('%') || trimmed.startsWith('//'))) {
...
function evalSum(atom: StoredAtom, db: StoredFact[], bindings: Bindings): Bindings[] {
  ...
  const matches: Array<{ groupKey: string; groupBindings: Bindings; aggValue: number }> = [];
...
function matchBody(body: StoredAtom[], db: StoredFact[], bindings: Bindings): Bindings[] {
  ...
  for (const fact of db) {
    ...
    if (nextBindings !== null) results.push(...matchBody(rest, db, nextBindings));
  }
  return results;
}
...
function matchBodyTracked(
  ...
  if (SIMPLE_BUILTINS.has(first.pred)) {
    ...
    const resolve = (term: Term): Constant => {
      ...
      if (isVariable(term)) return nextBindings.get(term) ?? bindings.get(term) ?? term;
      return parseConstant(term);
    };
      ...
      for (const { bindings, steps } of matchBodyTracked(rule.body, db, new Map(), [])) {
        ...
        if (!seen.has(key)) {
          ...
          provenance.set(key, {
            rule,
            groundBody: steps
              .filter((step): step is Extract<GroundBodyStep, { kind: 'fact' }> => step.kind === 'fact')
              .map((step) => step.fact),
          });
        }
      }
...
export class Datalog {
  ...
  checkConflicts(constraintName?: string): ConstraintViolation[] {
    ...
    if (constraintName) {
      constraints = constraints.filter((constraint) => constraint.name === constraintName);
    }
    if (constraints.length === 0) return [];

    const { db, provenance } = evaluateWithProvenance(this.facts, this.rules);
    return checkConstraints(db, constraints, provenance, new Set(this.facts.map(factKey)));
  }
}
